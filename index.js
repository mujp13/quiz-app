...
function renderQuestion() {
  const questionHtml = $(`
    <div class="question">
      ...
      <label> 
        <input type="radio" name="choice" id="a" required value="${STORE[index].options[0]}"> ${STORE[index].options[0]}
      </label>
      <label>
        <input type="radio" name="choice" id="b" value="${STORE[index].options[1]}"> ${STORE[index].options[1]} 
      </label>
      <label>
        <input type="radio" name="choice" id="c" value="${STORE[index].options[2]}"> ${STORE[index].options[2]} 
      </label>
      <label>
        <input type="radio" name="choice" id="d" value="${STORE[index].options[3]}"> ${STORE[index].options[3]} 
      </label>
  ...
  $('.questionBox').on('click', '.submitButton', function(event) {
    ...
    if (selected === correct) {
      $('input[name=choice]:checked')
        .closest('label')
        .append('<div class="right-answer">You got it right!</div>');
      score++;
      $('.score').text(`${score}/${qNum}`);
    } else {
      $('input[name=choice]:checked')
        .closest('label')
        .append(`<div class="wrong-answer">You got it wrong! <br>The answer is "${STORE[index].answer}"</br></div>`);
      $('.score').text(`${score}/${qNum}`);
    }
    $('#answer').hide();
